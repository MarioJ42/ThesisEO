                ...
                @forelse($slots->whereNotNull('vendor_id') as $slot)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-4 text-sm text-gray-800">
                            ...
                            <div class="text-xs text-gray-500 mt-0.5">{{ $slot->package_name ?? ($slot->vendor_category_id == 1 ? 'Venue / Hotel Reservation' : 'Custom Service') }}</div>
                        </td>
                        ...
                        <td class="px-4 py-4 text-sm">
                            <form action="{{ route($user->role . '.events.slots.status', ['event' => $event->id, 'slot' => $slot->id]) }}" method="POST" class="flex justify-center items-center gap-2 m-0">
                                @csrf @method('PUT')

                                <select name="vendor_contact_id" class="w-36 bg-white border border-gray-300 text-gray-900 text-xs rounded-md p-1.5 focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-colors">
                                ...
                                <div class="flex items-center shadow-sm rounded-md">
                                    <input type="number" name="meal_crew" value="{{ $slot->meal_crew ?? 0 }}" min="0" class="w-12 text-center text-xs border border-gray-300 rounded-l-md p-1.5 focus:ring-blue-500 focus:border-blue-500 z-10" style="border-right: none;">
                                    <span class="bg-gray-100 text-gray-500 text-xs px-2.5 py-1.5 border border-gray-300 rounded-r-md font-semibold">Pax</span>
                                </div>

                                <select name="status" class="w-28 bg-white border border-gray-300 text-gray-900 text-xs rounded-md p-1.5 focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-colors">
                                ...
                                <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-md text-xs font-bold transition-colors shadow-sm">Save</button>
                            </form>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500 bg-gray-50/50">No vendors assigned yet. Please assign vendors in the Vendor Planning tab first.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
