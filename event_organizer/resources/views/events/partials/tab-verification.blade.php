                                            ...
                                            <div class="flex flex-col gap-3">
                                                <div class="bg-blue-50/50 p-4 rounded-lg border border-blue-100 flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        @elseif($slot->vendor_id == 1)
                                                            <p class="text-xs text-gray-500 mb-2 italic">Custom Fenix EO Service</p>
                                                            <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white border border-gray-200 shadow-sm">
                                                                <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                                <span class="text-[10px] font-bold text-gray-700 uppercase">Custom Pricing</span>
                                                            </div>
                                                        @else
                                                ...
                                                <div class="flex items-center justify-between bg-gray-50 px-4 py-2 rounded-lg border border-gray-200">
                                                    <div class="flex flex-wrap gap-2">
                                                        ...
                                                        <button @click="openPriceModal({{ $slot->id }}, '{{ addslashes($slot->vendor_name) }}', '{{ addslashes($slot->package_name ?? 'Custom Service') }}', {{ $baseCost ?? 0 }}, {{ $slot->net_price ?? 0 }}, {{ $slot->deal_price ?? 0 }})"
