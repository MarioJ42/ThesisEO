<div x-show="activeTab === 'overview'" x-cloak>
    ...
    <div class="mb-8 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Event Cover Image (RSVP Hub)</h3>
        <form action="{{ route($user->role . '.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row gap-4 items-center m-0">
            @csrf @method('PUT')

            @if($event->cover_image)
                <div class="w-32 h-20 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200">
                    <img src="{{ asset('storage/' . $event->cover_image) }}" alt="Cover" class="w-full h-full object-cover">
                </div>
            @else
                <div class="w-32 h-20 rounded-lg flex-shrink-0 border-2 border-dashed border-gray-300 flex items-center justify-center bg-gray-50 text-gray-400 text-xs text-center p-2">
                    No Image
                </div>
            @endif

            <div class="flex-grow w-full">
                <input type="file" name="cover_image" accept="image/*" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 border border-gray-200 rounded-md p-1 focus:ring-blue-500 focus:border-blue-500 bg-white">
                <p class="text-[10px] text-gray-400 mt-1">Format: JPG, PNG. Max: 5MB. Orientation: Landscape recommended.</p>
            </div>

            <button type="submit" class="bg-gray-900 hover:bg-black text-white px-5 py-2.5 rounded-md text-sm font-bold w-full sm:w-auto whitespace-nowrap transition-all shadow-md">
                Upload Image
            </button>
        </form>
    </div>
                    ...
                    <table class="min-w-full w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($sessionVerifiedSlots as $slot)
                                @php
                                    $activePicName = $slot->pic_name ?: ($slot->contact_name ?: 'Not assigned');
                                    $activePicPhone = $slot->pic_phone ?: ($slot->contact_phone ?: '-');
                                @endphp
                                <tr class="hover:bg-blue-50/30 transition-colors">
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-600">
                                        {{ $activePicPhone }}</td>
                                    <td class="px-4 py-4 text-sm text-center font-bold text-gray-700">
                                        {{ $slot->meal_crew ?? 0 }}</td>
                                </tr>
                            @endforeach
                        </tbody>
