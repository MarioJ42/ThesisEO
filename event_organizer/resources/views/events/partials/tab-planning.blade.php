                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($sessionSlots as $slot)
                                <tr class="hover:bg-blue-50/30">
                                    ...
                                    <td class="px-4 py-4 text-sm">
                                        @if (($slot->vendor_id && $slot->vendor_package_id) || ($slot->vendor_id && $slot->vendor_category_id == 1) || $slot->vendor_id == 1)
                                            @php
                                                $selectedPkg = collect(
                                                ...
                                                $currentDealPrice =
                                                    $slot->deal_price > 0
                                                        ? $slot->deal_price
                                                        : (isset($selectedPkg) ? ($selectedPkg->price ?? 0) : 0);
                                                $currentNetPrice =
                                                    $slot->net_price > 0
                                                        ? $slot->net_price
                                                        : (isset($selectedPkg) ? ($selectedPkg->net_price ?? 0) : 0);
                                            ...
                                            <div class="flex flex-col sm:flex-row sm:items-center justify-between bg-white border border-gray-200 p-3 rounded-xl shadow-sm gap-3">
                                                <div class="flex flex-col">
                                                    ...
                                                    @if($slot->vendor_category_id == 1)
                                                        <span class="text-[11px] text-gray-500 font-medium italic">Venue / Hotel Reservation</span>
                                                    @elseif($slot->vendor_id == 1)
                                                        <span class="text-[11px] text-gray-500 font-medium italic">Custom Fenix EO Service</span>
                                                    @else
                                                ...
                                                <div class="flex items-center gap-2 mt-1.5">
                                                    ...
                                                    @endif
                                                    @else
                                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-md bg-gray-50 text-gray-700 border border-gray-200">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    @if($slot->vendor_category_id != 1)
                                                    <button @click="$dispatch('open-price-modal', { slot_id: '{{ $slot->id }}', vendor_name: @js($slot->vendor_name), package_name: @js($selectedPkg->name ?? ($slot->vendor_id == 1 ? 'Custom Service' : '')), base_cost: {{ $baseCost }}, net_price: {{ round($currentNetPrice) }}, deal_price: {{ round($currentDealPrice) }} })"
                                        ...
                                        @else
                                            ...
                                            <div class="flex gap-2 items-stretch w-full h-[36px]">
                                                ...
                                                @if ($slot->status === 'unassigned')
                                                    <form action="{{ route($user->role . '.events.slots.destroy', ['event' => $event->id, 'slot' => $slot->id]) }}" method="POST" class="h-full">
                                                        @csrf @method('DELETE')
                                                        <button type="submit"
