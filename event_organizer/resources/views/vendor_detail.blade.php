        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden md:flex space-x-8">
                    ...
                    <a href="{{ route('vendor') }}"
                        class="text-gray-900 border-b-2 border-gray-900 font-medium text-sm transition-colors px-1 py-2">Vendor</a>
                    @if (Auth::check() && Auth::user()->role === 'klien')
                        <a href="{{ route('client.events.index') }}"
                            class="text-gray-500 hover:text-gray-900 font-medium text-sm transition-colors px-1 py-2">My
                            Events</a>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    @else
                        <a href="{{ route('login') }}"
                            class="bg-gray-900 hover:bg-gray-800 text-white px-6 py-2.5 rounded-md text-sm font-semibold tracking-wide transition-all shadow-sm">LOG
                            IN</a>
                    @endauth
                </div>
            </div>
    ...
    <main class="pt-32 pb-24 min-h-screen" x-data="{ isModalOpen: false, selectedPackageId: null, selectedCategoryId: null }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-end mb-8">
                <a href="{{ route('vendor') }}"
                    class="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors group">
            ...
            <div
                class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-8 items-start md:items-center mb-16">
                <div
                    class="w-32 h-32 md:w-40 md:h-40 flex-shrink-0 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 flex items-center justify-center">
                    @if ($vendor->logo)
                ...
                <div class="flex-grow">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4 text-gray-500 text-sm mb-6">
                        @if ($vendor->instagram && $vendor->instagram !== '-')
                            <p class="flex items-center gap-2"><svg class="w-4 h-4 text-pink-500" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                ...
                                </svg> {{ $vendor->instagram }}</p>
                        @endif
                        @if ($vendor->address && $vendor->address !== '-')
                            <p class="flex items-center gap-2"><svg class="w-4 h-4 text-blue-500" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="mb-16">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    @foreach ($categoryPackages as $package)
                                        <div
                                            class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-shadow group flex flex-col">
                                            <h4
                                                class="text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors">
                                                {{ $package->name }}</h4>
                                            <p
                                                class="text-xl font-extrabold text-gray-900 mb-4 border-b border-gray-50 pb-4">
                                            ...
                                            @auth
                                                @if (Auth::user()->role === 'klien')
                                                    <button
                                                        @click="isModalOpen = true; selectedPackageId = {{ $package->id }}; selectedCategoryId = {{ $categoryId }};"
                                                        type="button"
                                                        class="mt-6 w-full py-2.5 px-4 bg-gray-900 hover:bg-blue-600 text-white text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
                ...
                @else
                    <div
                        class="text-center py-10 bg-white rounded-2xl border border-dashed border-gray-200 text-gray-500 text-sm">
            ...
            <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($portfolios as $portfolio)
                            <div class="group relative rounded-2xl overflow-hidden aspect-[4/5] bg-gray-100 shadow-sm">
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                ...
                @else
                    <div
                        class="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200 text-gray-500 text-sm">
        ...
        <div x-show="isModalOpen" style="display: none;"
            ...
            <div x-show="isModalOpen" x-transition.opacity class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm"
            ...
            <div x-show="isModalOpen" x-transition.enter="ease-out duration-300"
                ...
                <div class="mb-6 text-center">
                    <div
                        class="w-12 h-12 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                <form method="POST" action="{{ route('client.events.add_package_direct') }}">
                    ...
                    <div class="mb-4 text-left">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Event Title <span
                                class="text-red-500">*</span></label>
                        @if (isset($clientEvents) && $clientEvents->count() > 0)
                            <select name="event_id" required
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-gray-900 focus:border-gray-900 outline-none transition-all bg-white text-gray-700 cursor-pointer">
                        ...
                        @else
                            <div class="p-4 bg-yellow-50 text-yellow-700 rounded-xl border border-yellow-200 text-sm">
                                <p>You don't have any events in the planning stage. <a
                                        href="{{ route('client.events.index') }}"
                                        class="font-bold underline hover:text-yellow-800">Create an event first</a>.
                                </p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="mb-8 text-left">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Session <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="flex gap-4">
                        <button type="button" @click="isModalOpen = false"
                            class="flex-1 px-6 py-3 border border-gray-200 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition-colors">Cancel</button>
                        <button type="submit"
                            class="flex-1 px-6 py-3 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            @if (!isset($clientEvents) || $clientEvents->count() == 0) disabled @endif>Add to Event</button>
                    </div>
                </form>
            </div>
        </div>
