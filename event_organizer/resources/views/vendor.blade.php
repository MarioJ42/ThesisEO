...
<head>
    ...
    <title>Vendors - Fenix Event Organizer</title>
...
<body class="bg-gray-50 text-gray-800 antialiased selection:bg-blue-500 selection:text-white">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden md:flex space-x-8">
                    ...
                    <a href="{{ route('vendor') }}"
                        class="text-gray-900 font-medium text-sm border-b-2 border-gray-900 px-1 py-2">Vendor</a>
                </div>
    ...
    <main class="pt-32 pb-24 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @php
                $excludedCategories = [
                    'Baloon & Dove', 'Church', 'Church Decoration', 'Corsage', 'Flower Shower',
                    'Genset', 'Guest Lunch', 'Hair Styling', 'Headpiece', 'LCD', 'LED',
                    'Meal Crew', 'Misua & Angco Tea', 'Pyramid Fountain & Toast', 'Pyramid Fountian & Toast',
                    'Ring Box', 'Robe & Veil', 'Room Decoration', 'Tie'
                ];

                $excludedVendors = ['Pribadi', 'EO'];

                $displayCategories = isset($categories) ? $categories->filter(function($category) use ($excludedCategories) {
                    return !in_array($category->name, $excludedCategories);
                })->map(function($category) use ($excludedVendors) {
                    $filteredVendors = $category->vendors->filter(function($vendor) use ($excludedVendors) {
            ...
            <div x-data="{ selectedCategory: 'All', searchQuery: '' }" class="flex flex-col md:flex-row gap-8 items-start">
                ...
                <aside class="w-full md:w-64 flex-shrink-0 sticky top-28 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm max-h-[calc(100vh-8rem)] overflow-y-auto no-scrollbar">
                    ...
                    <ul class="space-y-1">
                        <li>
                            <button @click="selectedCategory = 'All'"
                                    :class="selectedCategory === 'All' ? 'text-blue-600 font-bold bg-blue-50' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'"
                            ...
                            @foreach($displayCategories as $category)
                                <li>
                                    <button @click="selectedCategory = {{ json_encode($category->name) }}"
                                            :class="selectedCategory === {{ json_encode($category->name) }} ? 'text-blue-600 font-bold bg-blue-50' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'"
                                            class="w-full text-left px-4 py-2.5 rounded-lg transition-colors text-sm">
                                        {{ $category->name }}
                                    </button>
                                </li>
                ...
                <div class="flex-grow w-full">
                    @if($displayCategories->count() > 0)
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                    @foreach($category->vendors as $vendor)
                                        <div x-show="searchQuery === '' || {{ json_encode(strtolower($vendor->name)) }}.includes(searchQuery.toLowerCase())"
                                            ...
                                            <div class="aspect-[4/3] bg-gray-100 flex items-center justify-center border-b border-gray-100">
                                            ...
                                            <div class="p-6 flex-grow flex flex-col">
                                                <h3 class="text-lg font-bold text-gray-900 mb-1 leading-tight group-hover:text-blue-600 transition-colors">{{ $vendor->name }}</h3>
                                                ...
                                                <button type="button" class="w-full py-2.5 px-4 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-xl transition-colors mt-auto flex items-center justify-center gap-2">
                        ...
                        @endforeach
                    @else
                        <div class="text-center text-gray-500 py-12 bg-white rounded-xl border border-dashed border-gray-300">
