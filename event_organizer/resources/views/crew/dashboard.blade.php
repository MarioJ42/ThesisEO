...
@section('title', 'Fenix Event Organizer')

@section('content')
<script>
    document.title = "Fenix Event Organizer";
    ...
    if (!favicon) {
     ...
     favicon.href = "{{ asset('images/logo-fenix1.png') }}";
</script>
        ...
        <div class="lg:col-span-2 space-y-10">
            <div>
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="bg-blue-100 text-blue-700 py-1 px-3 rounded-full text-xs font-bold">{{ isset($availableEvents) ? $availableEvents->count() : 0 }} Events</span>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if(isset($availableEvents))
                        @forelse($availableEvents as $evt)
                            <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_2px_15px_rgba(0,0,0,0.03)] hover:shadow-lg transition-all relative overflow-hidden group">
                                ...
                                <h3 class="font-serif text-2xl font-bold text-gray-900 leading-tight mb-2 mt-1">{{ $evt->title }}</h3>
                                ...
                                <div class="flex items-center gap-2 text-xs text-gray-500 font-medium bg-gray-50 p-2.5 rounded-lg mb-4">
                                    ...
                                    {{ \Carbon\Carbon::parse($evt->event_date)->format('D, d M Y') }}
                                </div>

                                <form action="{{ route('crew.jobs.apply', $evt->id) }}" method="POST" class="m-0">
                                    @csrf
            ...
            @if(isset($pendingRequests) && $pendingRequests->count() > 0)
                <div>
                    <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
                        ...
                        My Applications (Waiting Assignment)
                    </h2>
                    <div class="space-y-3">
                        @foreach($pendingRequests as $req)
                            <div class="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                                <div>
                                    <h4 class="font-bold text-gray-900 text-sm">Event: <span class="text-blue-600">{{ $req->title }}</span></h4>
                                    <p class="text-xs text-gray-500 mt-1">{{ \Carbon\Carbon::parse($req->event_date)->format('d M Y') }}</p>
                                </div>
                                <span class="bg-orange-100 text-orange-700 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider animate-pulse">Waiting Assignment</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    @if(isset($myEvents))
                        @forelse($myEvents as $event)
                            @php
                                $jobdeskUpper = strtoupper($event->jobdesk);
                                $isFD = str_starts_with($jobdeskUpper, 'FD') || str_contains($jobdeskUpper, 'FRONT DESK');
                            @endphp
