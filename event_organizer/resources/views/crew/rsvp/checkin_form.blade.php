...
<div class="relative max-w-xl mx-auto p-8 min-h-screen flex flex-col justify-center"
     x-data="checkinForm(@js($allGuests), {{ $event->id }})">
    ...
    <template x-if="activeGuest">
        <div class="w-full">
            <div class="text-center mb-10">
                ...
                <h4 class="text-3xl font-bold text-blue-600 mt-3" x-text="activeGuest.name"></h4>
            </div>

            <form :action="`{{ url('/crew/events/'.$event->id.'/rsvp/checkin') }}/${activeGuest.id}`" method="POST" class="space-y-6" @submit="submitForm($event)">
                @csrf
                <div class="bg-white rounded-[2rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-gray-50">
                    ...
                    <div class="flex justify-between items-center mb-8">
                        <div class="flex flex-col">
                            ...
                            <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-0.5" x-text="`Invited: ${activeGuest.pax_invited} Pax`"></span>
                        </div>
                            ...
                            <template x-for="(t, index) in titipan" :key="index">
                                <div class="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] text-gray-500 font-medium" x-text="t.phone_number || 'No Phone'"></span>
                                    </div>
    ...
    <div x-show="showSearch" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-cloak>
...
<script>
    function checkinForm(allGuestsList, eventId) {
        return {
            ...
            init() {
                ...
                let found = null;
                if (token) {
                    found = this.allGuests.find(g => g.barcode_token === token);
                } else if (guestId) {
                ...
                if (!found) {
                    ...
                    if (token) found = offlineGuests.find(g => g.barcode_token === token);
                    else if (guestId) found = offlineGuests.find(g => String(g.id) === String(guestId));
                }

                if (found) {
                    this.activeGuest = found;
                    this.pax = (['attending', 'checked_in'].includes(found.status) && found.pax_actual > 0) ? found.pax_actual : found.pax_invited;

                    const titipanHistory = this.allGuests.filter(g => g.titipan_by == found.id && g.angpao_titipan == 1);
                    if(titipanHistory.length > 0) {
                        this.titipan = titipanHistory.map(g => ({ id: g.id, name: g.name, phone_number: g.phone_number, qty: 1 }));
                    }
                } else {
                    alert('Guest data not found or invalid token!');
                    window.location.href = `/crew/events/${eventId}/rsvp/search`;
                }
            },
            ...
            get filteredGuests() {
                ...
                const dbMatches = this.allGuests.filter(g =>
                    g.name.toLowerCase().includes(searchLower) ||
                    (g.phone_number && g.phone_number.includes(searchLower))
                );

                const offlineGuests = JSON.parse(localStorage.getItem('offline_new_guests_' + eventId)) || [];
                const offlineMatches = offlineGuests.filter(g =>
                    g.name.toLowerCase().includes(searchLower) ||
                    (g.phone_number && g.phone_number.includes(searchLower))
                );

                return [...dbMatches, ...offlineMatches].slice(0, 5);
            },
            ...
            submitForm(event) {
                ...
                if (navigator.onLine && !String(this.activeGuest.id).startsWith('offline_')) {
                    event.target.submit();
                } else {
                    const storageKey = 'offline_checkin_' + eventId;
                    let offlineData = JSON.parse(localStorage.getItem(storageKey)) || [];

                    const existingIndex = offlineData.findIndex(d => d.guest_id === this.activeGuest.id);
                    if(existingIndex >= 0) {
