...
<body class="bg-gray-50 text-gray-800 antialiased" x-data="{
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
                ...
                <button @click="isCreateModalOpen = true"
                    class="bg-gray-900 hover:bg-gray-800 text-white px-6 py-3 rounded-lg text-sm font-semibold tracking-wide transition-all shadow-md flex items-center gap-2">
                    ...
                    <nav class="flex space-x-8 overflow-x-auto no-scrollbar" aria-label="Tabs">
                        @foreach (['all' => 'All Events', 'draft' => 'Draft', 'planning' => 'Planning', 'ongoing' => 'Ongoing', 'completed' => 'Completed', 'canceled' => 'Canceled'] as $key => $label)
                            <button @click="activeTab = '{{ $key }}'"
                                :class="activeTab === '{{ $key }}' ? 'border-gray-900 text-gray-900' :
                                ...
                                {{ $label }}
                                <span
                                    :class="activeTab === '{{ $key }}' ? 'bg-gray-900 text-white' :
                                        'bg-gray-100 text-gray-600'"
                                    class="py-0.5 px-2 rounded-full text-[10px] font-bold transition-colors">
                                    {{ $counts[$key] }}
                                </span>
                            </button>
                        @endforeach
                    ...
                    @foreach ($events as $event)
                        @php
                            $hasFenixGuestbook = \Illuminate\Support\Facades\DB::table('event_vendor')
                                ->join(
                                    'vendor_categories',
                                    'event_vendor.vendor_category_id',
                                    '=',
                                ...
                                ->exists();
                        @endphp
                        <div x-show="activeTab === 'all' || activeTab === '{{ $event->status }}'"
                            x-transition.opacity.duration.300ms
                            class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full relative overflow-hidden"
                            x-cloak>
                            <div
                                ...
                                @elseif($event->status == 'canceled') bg-red-500 @endif">
                            </div>
                            ...
                            <div class="mt-auto space-y-3 pl-2 bg-gray-50 p-4 rounded-xl">
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900">
                                        {{ $event->package ? $event->package->name : 'Custom Arrangement' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">
                                        Project Leader</p>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @else
                                            <div
                                                class="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-xs">
                                                ?</div>
                            ...
                            @if (!in_array($event->status, ['draft', 'canceled']))
                                <div class="mt-6 pl-2 flex flex-col gap-2">
                                    <a href="{{ route('client.events.manage', $event->id) }}"
                                        class="block w-full py-2.5 bg-white border border-gray-200 text-gray-900 text-center rounded-lg text-sm font-semibold hover:border-gray-900 transition-colors">
                                        View Details
                                    </a>
                                    @if ($hasFenixGuestbook)
                                        <a href="{{ route('client.events.analytics', $event->id) }}"
                                            class="block w-full py-2.5 bg-gray-900 text-white text-center rounded-lg text-sm font-semibold hover:bg-black transition-colors shadow-sm">
                                            View RSVP Analytics
                                        </a>
                                    @endif
                                </div>
            ...
            @else
                <div
                    class="col-span-full py-20 flex flex-col items-center justify-center bg-white rounded-2xl border border-dashed border-gray-300">
                    ...
                    <p class="text-gray-500 text-sm mb-6 text-center max-w-sm">You haven't planned any events with us
                        yet. Let's create your first unforgettable moment!</p>
                    <button @click="isCreateModalOpen = true"
                        class="bg-gray-900 hover:bg-gray-800 text-white px-6 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                        Plan New Event
                    </button>
                </div>
            @endif
    ...
    <div x-show="isCreateModalOpen" x-transition.opacity class="fixed inset-0 flex items-center justify-center p-4"
        style="z-index: 999; background-color: rgba(0, 0, 0, 0.75); backdrop-filter: blur(4px);" x-cloak>
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl" @click.away="isCreateModalOpen = false">
            ...
            <form action="{{ route('client.events.store') }}" method="POST">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        <label class="block mb-1.5 text-sm font-bold text-gray-900">Event Title <span
                                class="text-red-500">*</span></label>
                        <input type="text" name="title" required placeholder="Wedding of Groom & Bride"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 transition-colors">
                    </div>
                    <div>
                        <label class="block mb-1.5 text-sm font-bold text-gray-900">Event Date <span
                                class="text-red-500">*</span></label>
                        <input type="date" name="event_date" required
                            min="{{ \Carbon\Carbon::today()->addDays(21)->format('Y-m-d') }}"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 transition-colors">
                    </div>
                    <div>
                        <label class="block mb-1.5 text-sm font-bold text-gray-900">Select Package <span
                                class="text-red-500">*</span></label>
                        <select name="package_id" required x-model="selectedPackage"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 transition-colors">
                            <option value="" disabled selected>Choose a base arrangement</option>
                            @foreach ($packages as $package)
                                <option value="{{ $package->id }}">{{ $package->name }} (Start from Rp
                                    {{ number_format($package->base_price, 0, ',', '.') }})</option>
                            @endforeach
                            <option value="custom">Custom Arrangement (Build from scratch)</option>
                        </select>
                        <p class="text-[11px] text-gray-500 mt-2 leading-relaxed">By selecting a package, we will
                            automatically set up standard vendor slots for you. You can customize them later.</p>
                    </div>
                </div>
                <div class="flex justify-end gap-3 p-6 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
                    <button type="button" @click="isCreateModalOpen = false"
                        class="px-5 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">Cancel</button>
                    <button type="submit"
                        class="px-5 py-2.5 text-sm font-bold text-white bg-gray-900 rounded-xl hover:bg-gray-800 transition-colors shadow-md">Create
                        Event</button>
                </div>
            </form>
        </div>
            ...
            <div class="fixed inset-0 flex items-center justify-center p-4"
                ...
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 text-center">
                    ...
                    <form action="{{ route('password.force_change') }}" method="POST" class="text-left space-y-4">
                        ...
                        <div>
                            <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Confirm
                                New Password</label>
