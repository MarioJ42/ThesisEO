        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="max-w-3xl mx-auto bg-white rounded-3xl p-8 border border-gray-100 shadow-sm relative overflow-hidden mb-12">
                ...
                <div class="text-center mt-2">
                    <span class="inline-block px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full mb-4 bg-blue-50 text-blue-600 border border-blue-100">
                        Digital Guestbook
                    </span>
            ...
            <div x-data="{
                ...
                guestSearch: '',
                guestForm: { id: '', name: '', phone_number: '', pax_invited: 1, table_name: '', side: 'General', status: 'attending' },
                openEditGuestModal(id, name, phone, pax, table, side, status) {
                    ...
                    this.guestForm.side = side || 'General';
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    @if (!$isRsvpLocked)
                        <button @click="isGuestModalOpen = true" class="bg-gray-900 hover:bg-gray-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center justify-center gap-2">
                ...
                @if ($isRsvpLocked)
                    <div class="mb-6 bg-red-50 border border-red-200 p-5 rounded-2xl flex items-start gap-4 text-red-800 shadow-sm">
                        <svg class="w-6 h-6 flex-shrink-0 mt-0.5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                        <div>
                            <h4 class="font-bold text-base mb-1">RSVP is Locked</h4>
                            <p class="text-sm">Your event is in <strong>{{ $daysUntilEvent }} days</strong>. Guest data has been finalized and submitted to the Fenix EO production team. You can no longer add, edit, or remove guests.</p>
                        </div>
                    </div>
                @else
                    <div class="mb-6 bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-center gap-3 text-amber-800">
                        <svg class="w-5 h-5 flex-shrink-0 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <p class="text-sm font-medium">Guest input will be automatically locked <strong>7 days</strong> before the event ({{ \Carbon\Carbon::parse($event->event_date)->subDays(7)->format('d M Y') }}).</p>
                    </div>
                @endif
                ...
                <div class="flex flex-col lg:flex-row gap-3 mb-4">
                    <div class="relative flex-grow">
                        <input type="text" x-model="guestSearch" placeholder="Search guest name or phone" class="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-colors">
                    ...
                    <form method="GET" class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto flex-shrink-0">
                        <select name="guest_status" onchange="this.form.submit()" class="bg-white border border-gray-200 text-gray-700 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 py-3 px-4 shadow-sm font-semibold transition-colors cursor-pointer appearance-none">
                        ...
                        <select name="guest_sort" onchange="this.form.submit()" class="bg-white border border-gray-200 text-gray-700 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 py-3 px-4 shadow-sm font-semibold transition-colors cursor-pointer appearance-none">
                        ...
                        <table class="min-w-full w-full whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($guests as $guest)
                                    <tr class="hover:bg-gray-50/50 transition-colors" x-show="guestSearch === '' || '{{ strtolower(addslashes($guest->name . ' ' . $guest->phone_number)) }}'.includes(guestSearch.toLowerCase())">
                                        ...
                                        <td class="px-5 py-4 text-sm text-center font-semibold text-gray-700">{{ $guest->side ?? 'General' }}</td>
                                        ...
                                        @if (!$isRsvpLocked)
                                            <td class="px-5 py-4 text-sm text-center">
                                                <div class="flex justify-center gap-2">
                                                    <button @click="openEditGuestModal({{ $guest->id }}, '{{ addslashes($guest->name) }}', '{{ addslashes($guest->phone_number) }}', {{ $guest->pax_invited }}, '{{ addslashes($guest->table_name) }}', '{{ $guest->side ?? 'General' }}', '{{ $guest->status }}')" class="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">Edit</button>
                                                    <form action="{{ route('client.events.guests.destroy', ['event' => $event->id, 'guest' => $guest->id]) }}" method="POST" class="inline">
                                                        @csrf @method('DELETE')
                                                        <button type="button" data-form-id="delete-guest-{{ $guest->id }}" onclick="confirmDelete(this)" class="text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">Delete</button>
                                                    </form>
                                                </div>
                                            </td>
                                        @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ $isRsvpLocked ? 6 : 7 }}" class="px-5 py-12 text-center">
                ...
                <div x-show="isGuestModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto" style="display: none;" x-cloak>
                    <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl my-8" @click.away="isGuestModalOpen = false">
                        <div class="flex justify-between items-center p-6 border-b border-gray-100">
                            <h3 class="text-xl font-bold text-gray-900">Add New Guest</h3>
                            <button @click="isGuestModalOpen = false" class="text-gray-400 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full transition-colors">
                        ...
                        <form action="{{ route('client.events.guests.store', $event->id) }}" method="POST">
                            ...
                            <div class="p-6 space-y-5">
                                <div>
                                    ...
                                    <input type="text" name="name" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                </div>
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">WhatsApp Number</label>
                                    <input type="text" name="phone_number" placeholder="08123456789" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                </div>
                                <div class="flex gap-4">
                                    <div class="w-1/2">
                                        ...
                                        <input type="number" name="pax_invited" value="1" min="1" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                    </div>
                                    <div class="w-1/2">
                                        <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Table Name</label>
                                        <input type="text" name="table_name" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                    </div>
                                </div>
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Side</label>
                                    <select name="side" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm transition-colors appearance-none">
                            ...
                            <div class="flex justify-end p-6 border-t border-gray-100 gap-3 bg-gray-50/50 rounded-b-2xl">
                                <button type="button" @click="isGuestModalOpen = false" class="px-6 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</button>
                                <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-gray-900 rounded-xl hover:bg-black transition-colors shadow-sm">Save Guest</button>
                            </div>
                        </form>
                    </div>
                </div>

                <div x-show="isEditGuestModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto" style="display: none;" x-cloak>
                    <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl my-8" @click.away="isEditGuestModalOpen = false">
                        <div class="flex justify-between items-center p-6 border-b border-gray-100">
                            <h3 class="text-xl font-bold text-gray-900">Edit Guest</h3>
                            <button @click="isEditGuestModalOpen = false" class="text-gray-400 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        </div>
                        <form :action="'{{ url('/client/events/' . $event->id . '/guests') }}/' + guestForm.id" method="POST">
                            @csrf @method('PUT')
                            <div class="p-6 space-y-5">
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Guest Name <span class="text-red-500">*</span></label>
                                    <input type="text" name="name" x-model="guestForm.name" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                </div>
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">WhatsApp Number</label>
                                    <input type="text" name="phone_number" x-model="guestForm.phone_number" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                </div>
                                <div class="flex gap-4">
                                    <div class="w-1/2">
                                        <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Pax Invited <span class="text-red-500">*</span></label>
                                        <input type="number" name="pax_invited" x-model="guestForm.pax_invited" min="1" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                    </div>
                                    <div class="w-1/2">
                                        <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Table Name</label>
                                        <input type="text" name="table_name" x-model="guestForm.table_name" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50">
                                    </div>
                                </div>
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-700 uppercase tracking-wider">Side</label>
                                    <select name="side" x-model="guestForm.side" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50 appearance-none">
                                        <option value="General">General</option>
                                        <option value="Groom">Groom</option>
                                        <option value="Bride">Bride</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="status" x-model="guestForm.status" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50 appearance-none">
                            ...
                            <div class="flex justify-end p-6 border-t border-gray-100 gap-3 bg-gray-50/50 rounded-b-2xl">
                                <button type="button" @click="isEditGuestModalOpen = false" class="px-6 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Cancel</button>
                                <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors shadow-sm">Update Guest</button>
                            </div>
                        </form>
                    </div>
                </div>
