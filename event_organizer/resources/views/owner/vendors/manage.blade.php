...
@section('content')
    <div class="max-w-7xl mx-auto" x-data="{
        ...
        packageForm: { id: '', vendor_category_id: '', name: '', price: '', net_price: '', details: '' },

        openEditPackageModal(id, vendor_category_id, name, price, net_price, details) {
            ...
            this.packageForm.net_price = net_price;
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        <span
                            class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $vendor->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
        ...
        <div class="bg-white rounded-t-lg shadow-sm border-b border-gray-200">
            <nav class="flex space-x-8 px-6 overflow-x-auto" aria-label="Tabs">
                ...
                <button @click="activeTab = 'packages'"
                    :class="activeTab === 'packages' ? 'border-blue-500 text-blue-600' :
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                    Packages & Pricelist
                </button>
                <button @click="activeTab = 'portfolios'"
                    :class="activeTab === 'portfolios' ? 'border-blue-500 text-blue-600' :
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                    Portfolios
                </button>
        ...
        <div class="bg-white rounded-b-lg shadow-sm border border-t-0 border-gray-200 p-6">
            ...
            <div x-show="activeTab === 'contacts'" x-cloak>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="isContactModalOpen = true"
                        class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md text-sm font-semibold transition-colors">+
                        Add PIC</button>
                </div>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        @if ($contact->is_primary)
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">Primary</span>
                                        @else
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Alternative</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $contact->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium">
                                        <div class="flex justify-center gap-2">
                                            <button
                                                @click="openEditContactModal({{ $contact->id }}, '{{ addslashes($contact->name) }}', '{{ addslashes($contact->phone) }}', {{ $contact->is_primary ? '1' : '0' }}, {{ $contact->is_active ? '1' : '0' }})"
                                                class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded">Edit</button>

                                            <form id="delete-form-{{ $contact->id }}"
                                                action="{{ route('owner.vendors.contacts.destroy', $contact->id) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete('{{ $contact->id }}')"
                                                    class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-2 py-1 rounded">Delete</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
            ...
            <div x-show="activeTab === 'packages'" x-cloak>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="isPackageModalOpen = true"
                        class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md text-sm font-semibold transition-colors">+
                        Add Package</button>
                </div>
                    ...
                    @forelse($vendor->packages as $package)
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-5">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500 mt-1 mb-2">
                                        {{ $package->category->name ?? 'Uncategorized' }}</div>
                                </div>
                                <div class="flex gap-2">
                                    <button
                                        @click="openEditPackageModal({{ $package->id }}, '{{ $package->vendor_category_id }}', '{{ addslashes($package->name) }}', '{{ $package->price }}', '{{ $package->net_price }}', '{{ addslashes($package->details) }}')"
                                        class="text-blue-600 hover:text-blue-900 bg-blue-100 hover:bg-blue-200 p-1.5 rounded transition-colors">
                                    ...
                                    <form id="delete-package-form-{{ $package->id }}"
                                        action="{{ route('owner.vendors.packages.destroy', $package->id) }}" method="POST"
                                        class="inline">
                                        @csrf @method('DELETE')
                                        <button type="button" onclick="confirmDeletePackage('{{ $package->id }}')"
                                            class="text-red-600 hover:text-red-900 bg-red-100 hover:bg-red-200 p-1.5 rounded transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="text-sm text-gray-600 whitespace-pre-wrap">
                                {{ $package->details ?: 'No details provided.' }}</div>
                        </div>
                    @empty
                        <div
                            class="col-span-full border-2 border-dashed border-gray-200 rounded-lg p-10 text-center text-gray-500">
            ...
            <div x-show="activeTab === 'portfolios'" x-cloak>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="isPortfolioModalOpen = true"
                        class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md text-sm font-semibold transition-colors">+
                        Upload Photo</button>
                </div>
                    ...
                    @forelse($vendor->portfolios as $portfolio)
                        <div
                            ...
                            <div
                                ...
                                <form id="delete-portfolio-form-{{ $portfolio->id }}"
                                    action="{{ route('owner.vendors.portfolios.destroy', $portfolio->id) }}"
                                    method="POST">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmDeletePortfolio('{{ $portfolio->id }}')"
                                        class="bg-red-500 hover:bg-red-600 text-white p-2 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                            </path>
            ...
            <div class="relative w-full max-w-md bg-white rounded-lg shadow-xl" @click.away="isContactModalOpen = false">
                ...
                <form action="{{ route('owner.vendors.contacts.store', $vendor->id) }}" method="POST">
                        ...
                        <div class="flex gap-4">
                            <div class="w-1/2">
                                ...
                                <select name="is_primary"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full py-2.5 pl-3 pr-10">
                    ...
                    <div class="flex justify-end p-4 border-t gap-2">
                        <button type="button" @click="isContactModalOpen = false"
        ...
        <div x-show="isEditContactModalOpen"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" x-cloak>
            <div class="relative w-full max-w-md bg-white rounded-lg shadow-xl"
                @click.away="isEditContactModalOpen = false">
                <div class="flex justify-between items-center p-4 border-b">
                ...
                <form :action="'{{ url('/owner/vendors/contacts') }}/' + contactForm.id" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">PIC Name</label>
                            <input type="text" name="name" x-model="contactForm.name" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Phone Number</label>
                            <input type="text" name="phone" x-model="contactForm.phone" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        </div>
                        ...
                        <div class="flex gap-4">
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Role</label>
                                <select name="is_primary" x-model="contactForm.is_primary"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full py-2.5 pl-3 pr-10">
                                    <option value="0">Alternative</option>
                                    <option value="1">Primary Contact</option>
                                </select>
                            ...
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Status</label>
                                <select name="is_active" x-model="contactForm.is_active"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full py-2.5 pl-3 pr-10">
                                    <option value="1">Active</option>
                                    <option value="0">Inactive</option>
                                </select>
                    ...
                    <div class="flex justify-end p-4 border-t gap-2">
                        <button type="button" @click="isEditContactModalOpen = false"
                            class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Update
                            PIC</button>
                    </div>
                </form>
            </div>
        ...
        <div x-show="isPackageModalOpen"
            ...
            <div class="relative w-full max-w-lg bg-white rounded-lg shadow-xl" @click.away="isPackageModalOpen = false">
                ...
                <form action="{{ route('owner.vendors.packages.store', $vendor->id) }}" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Package Category <span
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Package Name <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="name" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        </div>
                        <div class="flex gap-4">
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Net Price (Rp) <span class="text-red-500">*</span></label>
                                <input type="number" name="net_price" required min="0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            </div>
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Public Price (Rp) <span class="text-red-500">*</span></label>
        ...
        <div x-show="isEditPackageModalOpen"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" x-cloak>
            <div class="relative w-full max-w-lg bg-white rounded-lg shadow-xl"
                @click.away="isEditPackageModalOpen = false">
                <div class="flex justify-between items-center p-4 border-b">
                ...
                <form :action="'{{ url('/owner/vendors/packages') }}/' + packageForm.id" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Package Category <span
                                    class="text-red-500">*</span></label>
                            <select name="vendor_category_id" x-model="packageForm.vendor_category_id" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Package Name <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="name" x-model="packageForm.name" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                        </div>
                        <div class="flex gap-4">
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Net Price (Rp) <span class="text-red-500">*</span></label>
                                <input type="number" name="net_price" x-model="packageForm.net_price" required min="0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            </div>
                            <div class="w-1/2">
                                <label class="block mb-2 text-sm font-medium text-gray-900">Public Price (Rp) <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Specifications / Details</label>
                            <textarea name="details" x-model="packageForm.details" rows="4"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"></textarea>
                        </div>
                    </div>
                    <div class="flex justify-end p-4 border-t gap-2">
        ...
        <div x-show="isPortfolioModalOpen"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" x-cloak>
            <div class="relative w-full max-w-md bg-white rounded-lg shadow-xl"
                @click.away="isPortfolioModalOpen = false">
                <div class="flex justify-between items-center p-4 border-b">
                ...
                <form action="{{ route('owner.vendors.portfolios.store', $vendor->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Title <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="title" required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                                placeholder="Wedding of Groom & Bride">
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Description</label>
                            <textarea name="description" rows="3"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                                placeholder="Write a captivating description"></textarea>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Select Images (Max 5 at once) <span
                                    class="text-red-500">*</span></label>
                    ...
                    <div class="flex justify-end p-4 border-t gap-2">
                        <button type="button" @click="isPortfolioModalOpen = false"
                            class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Upload</button>
                    </div>
                </form>
            </div>
        </div>
